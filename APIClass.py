...
class NFLApi:
    ...
    def get_scores_by_week(self, season: int, week: int) -> pd.DataFrame:
        ...
        if response.status_code == 200:
            ...
            df = df[["Week", "HomeTeam", "AwayTeam", "HomeScore", "AwayScore"]]
            df['Winner'] = np.where((df['HomeScore'].notna()) & (df['AwayScore'].notna()) & (df['HomeScore'] > df['AwayScore']),
                df['HomeTeam'],np.where((df['HomeScore'].notna()) & (df['AwayScore'].notna()),df['AwayTeam'],None))
            return df
        else:
